...
public class Renderer3D implements Wrapper {
    ...
    public static void renderCube(MatrixStack matrices, Vec3d pos, double size, boolean fill, Color fillColor, boolean outline, Color outlineColor) {
        if (outline) renderBoxOutline(matrices, new Box(pos.add(-0.5 * size, -0.5 * size,-0.5 * size),pos.add(0.5 * size, 0.5 * size,0.5 * size)), outlineColor);
        if (fill) renderBox(matrices, new Box(pos.add(-0.5 * size, -0.5 * size,-0.5 * size),pos.add(0.5 * size, 0.5 * size,0.5 * size)), fillColor);
    }

    public static void renderCube(MatrixStack matrices, Box box, boolean fill, Color fillColor, boolean outline, Color outlineColor) {
    ...
    private static Vec3d cameraTransform(Vec3d vec3d) {
        ...
        Vector4f vec = new Vector4f((float)(vec3d.x - cameraPos.x), (float)(vec3d.y - cameraPos.y), (float)(vec3d.z - cameraPos.z), 1.0f);
        vec.mul(matrix);

package ru.drughack.utils.fonts;
...
public class FontRenderer implements Closeable, Wrapper {
    ...
    public void drawText(MatrixStack matrices, OrderedText text, float x, float y, int color, boolean dropShadow) {
        ...
        if (DrugHack.getInstance().getModuleManager() != null) {
            x += DrugHack.getInstance().getModuleManager().getFontModule().xOffset.getValue().floatValue();
            y += DrugHack.getInstance().getModuleManager().getFontModule().yOffset.getValue().floatValue();
        }
        ...
        matrices.scale(1.0f / this.multiplier, 1.0f / this.multiplier, 0);

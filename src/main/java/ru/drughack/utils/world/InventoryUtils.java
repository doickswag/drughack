...
import ru.drughack.api.mixins.accesors.IClientPlayerInteractionManager;
...
public class InventoryUtils implements Wrapper {
    ...
    public static void switchBack(String mode, int slot, int previousSlot) {
        ...
        switch (mode) {
            case "Silent" -> {
                if (previousSlot == DrugHack.getInstance().getPositionManager().getServerSlot()) return;
    ...
    public static int findBestAxe(int start, int end) {
        ...
        for (int i = end; i >= start; i--) {
            ...
            if (stack.getItem() == Items.DIAMOND_AXE) diamondSlot = i;
            if (stack.getItem() == Items.IRON_AXE) ironSlot = i;
            if (stack.getItem() == Items.GOLDEN_AXE) goldenSlot = i;
            if (stack.getItem() == Items.STONE_AXE) stoneSlot = i;
            if (stack.getItem() == Items.WOODEN_AXE) woodenSlot = i;
        }
    ...
    public static void swap(String mode, int slot, int targetSlot) {
        switch (mode) {
            ...
            case "Swap" -> mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, indexToSlot(slot), targetSlot, SlotActionType.SWAP, mc.player);
        }
    }
    ...
    public static void switchSlot(String mode, int slot, int previousSlot) {
        if (mode.equalsIgnoreCase("None")) return;
        if (slot == -1 || previousSlot == -1 || slot == DrugHack.getInstance().getPositionManager().getServerSlot()) return;

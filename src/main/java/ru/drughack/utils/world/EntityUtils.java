...
public class EntityUtils implements Wrapper {
    ...
    public static double getSpeed(Entity entity, SpeedUnit unit) {
        ...
        if (unit == SpeedUnit.KILOMETERS) return (speed * 3.6 * DrugHack.getInstance().getWorldManager().getTimerMultiplier()) * 20;
        else return speed / 0.05 * DrugHack.getInstance().getWorldManager().getTimerMultiplier();
    }

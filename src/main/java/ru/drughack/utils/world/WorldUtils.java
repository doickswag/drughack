...
public class WorldUtils implements Wrapper {
    ...
    public static boolean canSeeOutline(Vec3d vec3d) {
        return canSeeOutline(vec3d.getX(), vec3d.getY(), vec3d.getZ());
    }

    public static boolean canSeeOutline(double x, double y, double z) {
        return mc.world.raycast(new RaycastContext(new Vec3d(mc.player.getX(), mc.player.getEyeY(), mc.player.getZ()), new Vec3d(x, y, z), RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.NONE, mc.player)).getType() == HitResult.Type.MISS;
    }
    ...
    public static void placeBlock(BlockPos position, Direction direction, Hand hand, Runnable runnable, boolean rotate, boolean crystalDestruction, boolean render) {
        ...
        if (rotate) DrugHack.getInstance().getRotationManager().addPacketRotation(RotationUtils.getRotations(vec3d.getX(), vec3d.getY(), vec3d.getZ()));
        ...
        DrugHack.getInstance().getWorldManager().getPlaceTimer().reset();
        ...
        if (render) {
            ...
            if (!DrugHack.getInstance().getRenderManager().getRenderPositions().contains(renderPosition)) DrugHack.getInstance().getRenderManager().getRenderPositions().add(renderPosition);
        }
    }
    ...
    public static List<BlockPos> getBoxes(Box box) {
        List<BlockPos> boxes = new ArrayList<>();
        for (int x = (int) Math.floor(box.minX); x < Math.ceil(box.maxX); x++)
            for (int y = (int) Math.floor(box.minY); y < Math.ceil(box.maxY); y++)
                for (int z = (int) Math.floor(box.minZ); z < Math.ceil(box.maxZ); z++)
                    boxes.add(new BlockPos(x, y, z));

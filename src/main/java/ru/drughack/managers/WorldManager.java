...
public class WorldManager implements Wrapper {
    ...
    @Getter private final TimerUtils placeTimer = new TimerUtils();
    ...
    public void onPacketReceive(EventPacketReceive event) {
        ...
        if (event.getPacket() instanceof EntityStatusS2CPacket packet && packet.getStatus() == 35) {
            ...
            DrugHack.getInstance().getEventHandler().post(new EventPlayerPop(player, pops));
        }
    }
    ...
    public void onTick(EventTick event) {
        ...
        for (PlayerEntity player : mc.world.getPlayers()) {
            ...
            DrugHack.getInstance().getEventHandler().post(new EventPlayerDeath(player));

...
public class ModelRenderer implements Wrapper {
    ...
    public static void renderModel(Entity entity, boolean staticEntity, float scale, float tickDelta, Render render) {
        ...
        if (renderer instanceof LivingEntityRenderer<?, ?, ?> && renderState instanceof LivingEntityRenderState state) ((ILivingEntityRenderer) renderer).drughack$render(state, matrices, CustomVertexConsumerProvider.INSTANCE, 15);
        if (renderer instanceof EndCrystalEntityRenderer crystalRenderer && renderState instanceof EndCrystalEntityRenderState state) crystalRenderer.render(state, matrices, CustomVertexConsumerProvider.INSTANCE, 15);
        matrices.push();
    }
    ...
    private static class CustomVertexConsumerProvider implements VertexConsumerProvider {
        ...
        public VertexConsumer getBuffer(RenderLayer layer) {
            if (layer instanceof IMultiPhase phase) ((IMultiPhaseParameters) (Object) phase.drughack$getParameters()).drughack$getTarget();
